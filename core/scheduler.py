...
def kirim_peringatan_jadwal_mingguan(bot):
    ...
    for i in range(7):
        ...
        if current_date.month == next_week_start.month:
            if slot_terisi_infra.get(current_date_str, 0) < 2:
                tanggal_kurang_infra.append(current_date_str)
            if slot_terisi_ce.get(current_date_str, 0) < 1:
...
def kirim_peringatan_h_minus_3(bot):
    ...
    pesan_peringatan = ""
    if terisi_infra < 2:
        pesan_peringatan += f"• 🚨 Slot *INFRA* masih kurang ({terisi_infra}/2 terisi).\n"
    if terisi_ce < 1:
        pesan_peringatan += f"• 🚨 Slot *CE* masih kurang ({terisi_ce}/1 terisi).\n"
