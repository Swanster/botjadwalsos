        ...
        for day in week:
            ...
            # Logika untuk mode pengisian jadwal & cuti (yang sudah ada sebelumnya)
                ...
                if mode == 'jadwal':
                    ...
                    kuota_harian = int(get_setting('kuota_infra', '2')) if user_group == 'INFRA' else int(get_setting('kuota_ce', '1'))
                    jumlah_terisi = slot_terisi_infra.get(current_date_str, 0) if user_group == 'INFRA' else slot_terisi_ce.get(current_date_str, 0)
                    
                    is_penuh = jumlah_terisi >= kuota_harian
        ...
        # --- BAGIAN 3: Logika untuk Interaksi Kalender 'Jadwal' atau 'Cuti' ---
            ...
            if action == 'toggle':
                ...
                if mode == 'jadwal' and date_str not in selections:
                    ...
                    # --- LOGIKA BARU: Batasan dinamis per grup dari database ---
                    user_group = get_user_group(user_id)
